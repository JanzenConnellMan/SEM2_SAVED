...
#Table  <- table(factor(Mat.S2, levels = 1:S))/(dm*dm)
#TableV <- as.vector(Table)

#df.PropsM <- as.matrix(df.Props)



#write.csv(df.PropsM,"Test_v5_A25.csv",quote=F,row.names=F)
#write.csv(Mat.S,"TestT_v5_A25",quote=F,row.names=F)
